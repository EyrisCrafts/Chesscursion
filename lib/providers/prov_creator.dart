...
class ProvCreator extends ChangeNotifier {
  ...
  List<List<EnumBoardPiece>> translatedMainBoard = [];
  ...
  void saveBoard() {
    List<List<int>> raw = Utils.convertBoardToRaw(translatedMainBoard);
    log(raw.toString());
  }

  void restartBoard() {
    translatedMainBoard = Utils.convertBoard(emptyBoard.map((element) => List<int>.from(element)).toList());
